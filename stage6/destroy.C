...
const Status RelCatalog::destroyRel(const string &relation) {
    ...
    if (relation.empty() || relation == string(RELCATNAME) || relation == string(ATTRCATNAME)) return BADCATPARM;
...
const Status AttrCatalog::dropRelation(const string &relation) {
    ...
    for (i = 0; i < attrCnt; i++) {
        if ((status = removeInfo(relation, attrs[i].attrName)) != OK) return status;
    }
