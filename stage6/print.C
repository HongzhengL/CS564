...
const Status UT_Print(string relation) {
    ...
    // get attribute data
    if ((status = attrCat->getRelInfo(rd.relName, attrCnt, attrs)) != OK) return status;

    // compute width of output columns

    int *attrWidth;
    if ((status = UT_computeWidth(attrCnt, attrs, attrWidth)) != OK) return status;
    ...
    for (i = 0; i < attrCnt; i++) {
        for (int j = 0; j < attrWidth[i]; j++) putchar('-');
        printf("  ");
    }
    printf("\n");

    if ((status = hfile->startScan(0, 0, INTEGER, NULL, EQ)) != OK) return status;
    ...
    while ((status = hfile->scanNext(rid)) == OK) {
        if ((status = hfile->getRecord(rec)) != OK) return status;
    ...
    if ((status = hfile->endScan()) != OK) return status;
    delete hfile;
