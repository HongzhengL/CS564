...
const Status AttrCatalog::getInfo(const string &relation, const string &attrName, AttrDesc &record) {
    Status status;
    RID rid;
    Record rec;
    HeapFileScan *hfs;
    ...
    if (status == FILEEOF) status = ATTRNOTFOUND;
...
const Status AttrCatalog::removeInfo(const string &relation, const string &attrName) {
    Status status;
    Record rec;
    RID rid;
    AttrDesc record;
    ...
    while ((status = hfs->scanNext(rid)) == OK) {
        ...
        memcpy(&record, rec.data, rec.length);
#ifdef DEBUGCAT
        cerr << "%%  Read attrcat entry " << record.relName << "." << record.attrName << endl;
#endif
        if (string(record.attrName) == attrName) break;
    }
    if (status == FILEEOF) status = RELNOTFOUND;
    if (status == OK) {
#ifdef DEBUGCAT
        cout << "%%  Deleting attrcat entry " << record.relName << "." << record.attrName << endl;
#endif
...
const Status AttrCatalog::getRelInfo(const string &relation, int &attrCnt, AttrDesc *&attrs) {
    Status status;
    RID rid;
    Record rec;
    HeapFileScan *hfs;
    ...
    while ((status = hfs->scanNext(rid)) == OK) {
        ...
        ++attrCnt;
        if (attrCnt == 1) {
            if (!(attrs = (AttrDesc *)malloc(sizeof(AttrDesc)))) return INSUFMEM;
        } else {
            if (!(attrs = (AttrDesc *)realloc(attrs, attrCnt * sizeof(AttrDesc)))) return INSUFMEM;
        }
