...
const Status RelCatalog::createRel(const string &relation, const int attrCnt, const attrInfo attrList[]) {
    Status status;
    RelDesc rd;
    AttrDesc ad;
    ...
    if (relation.length() >= sizeof rd.relName) return NAMETOOLONG;
    ...
    if (attrCnt > 1) {
        for (int i = 1; i < attrCnt; i++) {
            ...
            for (int j = 0; j < i; j++)
                if (strcmp(attrList[i].attrName, attrList[j].attrName) == 0) return DUPLATTR;
        }
    }
    ...
    rd.attrCnt = attrCnt;
    if ((status = addInfo(rd)) != OK) return status;
    ...
    for (int i = 0; i < attrCnt; i++) {
        if (strlen(attrList[i].attrName) >= sizeof ad.attrName) return NAMETOOLONG;
