...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (status != OK) {
            ...
            cout << "inserted " << i << " records into file dummy1 before error " << endl;
            error.print(status);
            exit(1);
        }
    }
    ...
    if (status != OK)
        ...
        if (count != num / 2 + num)
            cout << "Err0r.   scan should have returned " << num / 2 + num << " records!" << endl;
    ...
    if (status != OK)
        ...
        if (numDeletes != 1000) cout << "Err0r. Should have deleted 1000 records!!!" << endl;
    }
    cout << endl;
    delete scan1;
    ...
    if (status != OK) {
        ...
        cout << "scan file1 saw " << i << " records " << endl;
        if (i != num / 10 - 1)
            cout << "Err0r.   filtered scan 2 should have returned " << num / 10 - 1 << " records!" << endl;
    }
    delete scan1;
